...
class llama_memory_hybrid_hca : public llama_memory_i {
public:
    llama_memory_hybrid_hca(
            const llama_model & model,
                    ggml_type   type_k,
                    ggml_type   type_v,
                         bool   v_trans,
    ...
    llama_kv_cache * get_full() const;
    llama_kv_cache_dsv4 * get_hca() const;
    llama_memory_recurrent * get_recr() const;

private:
    uint32_t n_rs_seq;
    std::unique_ptr<llama_kv_cache> mem_full;
    std::unique_ptr<llama_kv_cache_dsv4> mem_hca;
    std::unique_ptr<llama_memory_recurrent> mem_recr;
};

class llama_memory_hybrid_hca_context : public llama_memory_context_i {
public:
    explicit llama_memory_hybrid_hca_context(llama_memory_status status);
    llama_memory_hybrid_hca_context(llama_memory_hybrid_hca * mem, llama_context * lctx, bool optimize);
    llama_memory_hybrid_hca_context(
            llama_memory_hybrid_hca * mem,
            llama_memory_context_ptr ctx_full,
            llama_memory_context_ptr ctx_hca,
            std::vector<llama_ubatch> ubatches);
    ...
    const llama_kv_cache_context * get_full() const;
    const llama_kv_cache_dsv4_context * get_hca() const;
    const llama_memory_recurrent_context * get_recr() const;
